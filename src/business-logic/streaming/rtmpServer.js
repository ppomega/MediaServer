...
function ffmpeg(session) {
  ...
  fs.mkdirSync(`./${session.searchParams.get("name")}/${streamName}/dash`);
  exec(
    ...
    (err, stdout, stderr) => {
      ...
      fs.unwatchFile(
        `${session.searchParams.get("name")}/${streamName}/dash/manifest.mpd`
      );
      fs.unlinkSync(
        `${session.searchParams.get("name")}/${streamName}/test.flv`
      );
      app.emit("RecordComplete", streamName, session.searchParams.get("name"));
    }
  );
  exec(
...
app.on("RecordComplete", async (streamName, name) => {
  const C = new Client({
  ...
  const files = await fsPromises.readdir(`./${name}/${streamName}`, {
    recursive: true,
  });
  async function uploadFiles() {
    for (const file of files) {
      ...
      await C.UploadData.smallUpload(
        path.resolve(`./${name}/${streamName}`, file),
        name + "/" + streamName + "/" + file.replace(/\\/g, "/")
      );
    }
  }
  await uploadFiles();
  await lect_update(streamName, name, 1);
